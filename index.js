...
async function redisSave(token, entry) {
  if (!redis) return;
  try { await redis.set('lv:token:' + token, JSON.stringify({ createdAt: entry.createdAt, lastUsed: entry.lastUsed, reqCount: entry.reqCount, piKey: entry.piKey || null, piSecret: entry.piSecret || null })); }
...
async function getTokenEntry(token) {
  ...
  const entry = { createdAt: saved.createdAt, lastUsed: saved.lastUsed, reqCount: saved.reqCount, rateWin: [], piKey: saved.piKey || null, piSecret: saved.piSecret || null };
...
function normalizeQuery(q) { return cleanText(q).replace(/^(the|a|an)\s+/i, '').trim(); }
function normalizeLoose(s) {
  return normalizeQuery(String(s || '').toLowerCase())
    .replace(/&/g, ' and ')
    .replace(/[^a-z0-9]+/g, ' ')
    .replace(/\b(vol|volume|part|chapter|book|audiobook|audiobooks|unabridged|dramatic|reading|read by)\b/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}
function tokenizeLoose(s) { return normalizeLoose(s).split(/\s+/).filter(Boolean); }
function quotedVariants(q) {
  const out = new Set();
  const raw = cleanText(q);
  const norm = normalizeQuery(raw);
  if (raw) out.add(raw);
  if (norm && norm !== raw) out.add(norm);
  const byMatch = raw.match(/^(.+?)\s+by\s+(.+)$/i);
  if (byMatch) {
    out.add(cleanText(byMatch[1]));
    out.add(cleanText(byMatch[2]));
  }
  const loose = tokenizeLoose(raw);
  if (loose.length >= 2) out.add(loose.join(' '));
  if (loose.length >= 3) out.add(loose.slice(0, 3).join(' '));
  if (loose.length >= 4) out.add(loose.slice(0, 4).join(' '));
...
async function searchBooks(q) {
  const variants = quotedVariants(q);
  const byMatch = cleanText(q).match(/^(.+?)\s+by\s+(.+)$/i);
  const titlePart = byMatch ? cleanText(byMatch[1]) : '';
  const authorPart = byMatch ? cleanText(byMatch[2]) : '';

  const requests = [
    ...variants.map(tq => lvGet('/audiobooks', { title: tq, limit: 12, extended: 1, coverart: 1 })),
    lvGet('/audiobooks', { title: cleanText(q), limit: 18, extended: 1, coverart: 1 }),
    lvGet('/audiobooks', { author: cleanText(q), limit: 10, extended: 1, coverart: 1 })
  ];
  if (titlePart && authorPart) {
    requests.push(lvGet('/audiobooks', { title: titlePart, author: authorPart, limit: 14, extended: 1, coverart: 1 }));
    requests.push(lvGet('/audiobooks', { author: authorPart, limit: 20, extended: 1, coverart: 1 }));
  }
  ...
  for (const data of results) {
    for (const book of lvBooks(data)) {
      ...
      const hay = [title, author, cleanText(book.description || '')].join(' ');
      const score = scoreResult(q, title, author, 'LibriVox', hay);
      if (overlapScore(q, hay) < 0.45 && score < 70) continue;
      const prev = byId.get(bid);
      if (!prev || score > prev._score) byId.set(bid, { ...book, _score: score });
    }
  }
  return Array.from(byId.values()).sort((a, b) => (b._score || 0) - (a._score || 0));
}
...
function hasPodcastIndex(entry) {
  return !!(entry && entry.piKey && entry.piSecret);
}

function podcastIndexHeaders(entry) {
  const ts = Math.floor(Date.now() / 1000).toString();
  const auth = crypto.createHash('sha1').update(entry.piKey + entry.piSecret + ts).digest('hex');
  return {
    'User-Agent': UA,
    'X-Auth-Date': ts,
...
async function piGet(entry, endpoint, params = {}) {
  if (!hasPodcastIndex(entry)) return null;
  try {
    const r = await fetchWithTimeout(PI_BASE + endpoint, {
      params,
      headers: podcastIndexHeaders(entry)
    }, SOURCE_TIMEOUT_MS);
    return r.data;
  } catch (e) {
    console.warn('[PI]', endpoint, e.message);
    return null;
  }
}

async function piValidate(piKey, piSecret) {
  const entry = { piKey, piSecret };
  const data = await piGet(entry, '/search/byterm', { q: 'test', max: 1, pretty: false });
  return !!(data && (data.status === true || data.status === 'true' || Array.isArray(data.feeds)));
}

function isAudiobookLikeFeed(feed, query) {
  const medium = String(feed.medium || '').toLowerCase();
  const title = cleanText(feed.title || '');
  const author = cleanText(feed.author || feed.itunesAuthor || '');
  const desc = cleanText(feed.description || feed.descriptionOriginal || '');
  const hay = [title, author, desc].join(' ').toLowerCase();
  const positive = /(audiobook|audiobooks|librivox|public domain|chapters?|chapter\s+\d+|book\s+\d+|read by|narrated by|complete book|full book|full audiobook)/i.test(hay);
  const negative = /(podcast|news|daily|weekly|episode|interview|sermon|radio|mix|show|talk|hour|minutes?\b|review|recap|trailer)/i.test(hay);
  const overlap = overlapScore(query, hay);
  return /audio/.test(medium) && positive && (!negative || overlap >= 0.75);
}

async function podcastIndexSearch(entry, q) {
  if (!hasPodcastIndex(entry)) return [];
  try {
    const data = await piGet(entry, '/search/byterm', { q, max: 20, pretty: false });
    const feeds = Array.isArray((data || {}).feeds) ? data.feeds : [];
    return feeds
      .filter(f => isAudiobookLikeFeed(f, q))
      .map(f => ({
        id: 'pi_feed_' + String(f.id),
        title: cleanText(f.title || f.itunesAuthor || 'Audiobook Feed'),
        artist: cleanText(f.author || f.itunesAuthor || 'Podcast Index'),
        artworkURL: safeUrl(f.artwork || f.image || f.imageUrl),
        feedId: String(f.id),
        source: 'Podcast Index',
        description: cleanText(f.description || f.descriptionOriginal || '')
      }));
  } catch (e) {
    console.warn('[PI search]', e.message);
    return [];
  }
}

async function podcastIndexAlbum(entry, id) {
  if (!hasPodcastIndex(entry)) return null;
  try {
    const feedId = id.replace('pi_feed_', '');
    const data = await piGet(entry, '/episodes/byfeedid', { id: feedId, max: 300, pretty: false });
    const items = Array.isArray((data || {}).items) ? data.items : [];
    if (!items.length) return null;
    const feedTitle = cleanText(items[0].feedTitle || 'Audiobook Feed');
    const artist = cleanText(items[0].feedAuthor || 'Podcast Index');
    const artwork = safeUrl(items[0].feedImage || items[0].image);
    const tracks = items
      .map((ep, idx) => ({
        id: 'pi_ep_' + String(ep.id || (feedId + '_' + idx)),
        title: cleanText(ep.title || ('Chapter ' + (idx + 1))),
        artist,
        album: feedTitle,
        duration: ep.duration ? parseInt(ep.duration) || null : null,
        artworkURL: artwork,
        streamURL: safeUrl(ep.enclosureUrl),
        format: (/\.m4a$/i.test(String(ep.enclosureUrl || '')) ? 'm4a' : /\.ogg$/i.test(String(ep.enclosureUrl || '')) ? 'ogg' : 'mp3'),
        _text: cleanText([ep.title, ep.description, ep.summary].filter(Boolean).join(' '))
      }))
      .filter(t => t.streamURL)
      .filter(t => !/(trailer|preview|sample|teaser|promo|interview|review|episode\s*\d+)/i.test(t.title + ' ' + t._text));
    if (tracks.length < 2) return null;
    tracks.forEach(cacheChapter);
    return { id, title: feedTitle, artist, artworkURL: artwork, trackCount: tracks.length, tracks: tracks.map(({ _text, ...rest }) => rest) };
  } catch (e) {
    console.warn('[PI album]', e.message);
    return null;
  }
}
...
async function loyalAlbum(id) {
  try {
    ...
    const tracks = items.map((item, idx) => {
      ...
      const dur = parseHMS(getXmlValue(item, 'itunes:duration') || getXmlValue(item, 'duration'));
      const epAuthor = cleanText(getXmlValue(item, 'itunes:author') || getXmlValue(item, 'author')) || author;
      return {
        ...
        format: /\.m4a$/i.test(String(en || '')) ? 'm4a' : /\.ogg$/i.test(String(en || '')) ? 'ogg' : 'mp3'
      };
    }).filter(t => t.streamURL);
...
async function openCultureSearch(q) {
  try {
    ...
    const seen = new Set();
    let m;
    while ((m = linkRx.exec(html)) && out.length < 20) {
      const href = safeUrl(m[1]);
      const text = cleanText(stripTags(m[2]));
      if (!href || !text) continue;
      if (text.length < 3 || text.length > 180) continue;
      const score = scoreResult(q, text, 'Open Culture', 'Open Culture', href);
      if (overlapScore(q, text) < 0.6 && score < 70) continue;
    ...
    return out.sort((a, b) => (b._score || 0) - (a._score || 0)).slice(0, 10).map(({ _score, ...rest }) => rest);
  } catch (e) {
...
async function iaSearch(q) {
  try {
    ...
    const params = {
      q: `mediatype:(audio) AND (${q}) AND (collection:librivoxaudio OR subject:(audiobook OR audiobooks) OR title:(audiobook))`,
      fl: 'identifier,title,creator,description,collection,mediatype,subject',
      sort: 'downloads desc',
      rows: 18,
      page: 1,
      output: 'json'
    ...
    const words = cleanText(q).toLowerCase().split(/\s+/).filter(Boolean);
    const scored = docs.map(d => {
      const title = cleanText(d.title || d.identifier);
      const creator = cleanText(Array.isArray(d.creator) ? d.creator.join(', ') : (d.creator || 'Internet Archive'));
      const hay = (title + ' ' + creator + ' ' + cleanText(Array.isArray(d.subject) ? d.subject.join(' ') : (d.subject || ''))).toLowerCase();
      let score = 0;
      for (const w of words) {
        if (title.toLowerCase().includes(w)) score += 4;
        else if (hay.includes(w)) score += 1;
      }
      if (String(d.collection || '').toLowerCase().includes('librivox')) score += 3;
      if (/audiobook/i.test(hay)) score += 2;
      return {
    ...
    }).filter(x => x._score >= Math.max(2, words.length));
    return scored.sort((a, b) => b._score - a._score).slice(0, 10).map(({ _score, ...rest }) => rest);
  } catch (e) {
...
function iaFileDuration(f) {
  return parseHMS(f.length || f.duration || f.runtime || f.time || '');
}
function iaFileArtist(f, fallbackArtist) {
  return cleanText(f.creator || f.artist || fallbackArtist || 'Internet Archive');
}
function iaLooksLikeFullBook(title, name, duration) {
  const hay = cleanText([title, name].join(' ')).toLowerCase();
  if (duration && duration >= 4 * 3600) return true;
  return /(complete|full|unabridged|audiobook|entire book)/i.test(hay);
}
function iaLooksLikeChapter(title, name, trackNo) {
  const hay = cleanText([title, name, trackNo].join(' ')).toLowerCase();
  return /(chapter|part|track|disc|cd)\s*(\d+|[ivxlcdm]+)/i.test(hay);
}
async function iaAlbum(id) {
  try {
    ...
    const artist = cleanText(Array.isArray(meta.creator) ? meta.creator.join(', ') : (meta.creator || meta.creatorSorter || 'Internet Archive'));
    const artwork = 'https://archive.org/services/img/' + identifier;
    const audioFiles = files.filter(f => {
      const name = String(f.name || '').toLowerCase();
      const format = String(f.format || '').toLowerCase();
      return (/\.(mp3|m4a|ogg|flac)$/i.test(name) || /(mp3|mpeg|vbr mp3|64kbps mp3|audio|ogg|flac|m4a)/i.test(format)) && !/\.(jpg|jpeg|png|gif|txt|xml|m3u|pls|cue|sqlite|db)$/i.test(name) && !/_spectrogram\./i.test(name);
    });

    const chapterish = audioFiles.filter(f => iaLooksLikeChapter(f.title || '', f.name || '', f.track || f.tracknum || ''));
    const fullbookish = audioFiles.filter(f => iaLooksLikeFullBook(f.title || '', f.name || '', iaFileDuration(f)));
    let chosen = chapterish.length >= 2 ? chapterish : audioFiles;
    if (chapterish.length >= 2 && fullbookish.length) {
      const fullNames = new Set(fullbookish.map(f => String(f.name || '')));
      chosen = chapterish.filter(f => !fullNames.has(String(f.name || '')));
      if (!chosen.length) chosen = chapterish;
    }

    const tracks = chosen.slice(0, 300).map((f, idx) => ({
      id: 'ia_tr_' + identifier + '_' + idx,
      title: cleanText(f.title || f.name || ('Track ' + (idx + 1))),
      artist: iaFileArtist(f, artist),
      album: title,
      duration: iaFileDuration(f),
      artworkURL: artwork,
      streamURL: f.name ? ('https://archive.org/download/' + encodeURIComponent(identifier) + '/' + encodeURIComponent(f.name)) : null,
...
function tokenize(q) {
  return cleanText(q).toLowerCase().split(/\s+/).filter(Boolean);
}

function scoreResult(query, title, artist, source, extra = '') {
  const q = normalizeLoose(query);
  const t = normalizeLoose(title);
  const a = normalizeLoose(artist);
  const e = normalizeLoose(extra);
  ...
  for (const w of words) {
    if (t === w) score += 18;
    else if (t.includes(w)) score += 13;
    else if (a.includes(w)) score += 5;
    else if (e.includes(w)) score += 3;
    else score -= 8;
  }
  const ov = overlapScore(query, [title, artist, extra].join(' '));
  score += Math.round(ov * 60);
  if (/\b(soundtrack|score|theme|remix|cover|mix|instrumental|karaoke|summary|analysis|review|discussion|podcast)\b/i.test((title || '') + ' ' + (extra || ''))) score -= 50;
  if (words.length >= 2 && ov < 0.5) score -= 40;
  if (source === 'LibriVox') score += 10;
  if (source === 'Loyal Books') score += 7;
  if (source === 'Internet Archive') score += 4;
  if (source === 'Open Culture') score -= 4;
  if (source === 'Project Gutenberg') score -= 30;
  if (source === 'Podcast Index') score -= 10;
  return score;
}

function annotateAndSort(query, items) {
  return (items || [])
    .map(x => ({ ...x, _score: scoreResult(query, x.title || x.name || '', x.artist || x.creator || '', x.source || '', [x.description || '', x.album || ''].join(' ')) }))
    .filter(x => (x._score || 0) >= 35)
    .sort((a, b) => b._score - a._score);
}

function mergeDuplicateAlbums(items) {
  const map = new Map();
  for (const item of (items || [])) {
    const key = (cleanText(item.title) + '|' + cleanText(item.artist)).toLowerCase();
    const prev = map.get(key);
    if (!prev || (item._score || 0) > (prev._score || 0)) map.set(key, item);
  }
...
async function gutenbergSearch(q) {
  try {
    const r = await fetchWithTimeout('https://www.gutenberg.org/ebooks/search/?query=' + encodeURIComponent(q), {
      headers: { 'User-Agent': UA, 'Accept': 'text/html,application/xhtml+xml' }
    });
    const html = String(r.data || '');
    const rx = /<li class="booklink">([\s\S]*?)<\/li>/gi;
    const out = [];
    let m;
    while ((m = rx.exec(html)) && out.length < 10) {
      const block = m[1];
      const href = (block.match(/href="([^"]+)"/i) || [,''])[1];
      const title = stripTags((block.match(/class="title"[^>]*>([\s\S]*?)<\/span>/i) || [,''])[1]);
      const subtitle = stripTags((block.match(/class="subtitle"[^>]*>([\s\S]*?)<\/span>/i) || [,''])[1]);
      if (!href || !title) continue;
      out.push({
        id: 'pg_book_' + Buffer.from(href).toString('base64url').slice(0, 48),
        title: cleanText(title),
        artist: cleanText(subtitle || 'Project Gutenberg'),
        artworkURL: null,
        sourcePage: 'https://www.gutenberg.org' + href,
        source: 'Project Gutenberg'
      });
    }
    return out;
  } catch (e) {
    console.warn('[PG search]', e.message);
    return [];
  }
}

async function podcastEpisodeSearch(q) {
  return [];
}

async function multiSourceSearch(entry, q) {
  const [lvBooksFound, loyalBooks, iaBooks, ocBooks, pgBooks, piFeeds] = await Promise.all([
    searchBooks(q),
    loyalSearch(q),
    iaSearch(q),
    openCultureSearch(q),
    gutenbergSearch(q),
    podcastIndexSearch(entry, q)
  ]);
  ...
  const otherAlbums = [
    ...
    ...pgBooks.map(b => ({ id: b.id, title: b.title, artist: b.artist, artworkURL: b.artworkURL, trackCount: null, year: null, source: 'Project Gutenberg' })),
    ...piFeeds.map(b => ({ id: b.id, title: b.title, artist: b.artist, artworkURL: b.artworkURL, trackCount: null, year: null, source: 'Podcast Index' }))
  ];

  const lvTracks = lvBooksFound.slice(0, 10).map(mapBookToTrack).map(x => ({ ...x, source: 'LibriVox' }));
  const instantTracks = [
    ...loyalBooks.slice(0, 8).map(b => ({ id: b.id, title: b.title, artist: b.artist || 'Unknown Author', album: b.title, duration: null, artworkURL: b.artworkURL, streamURL: null, format: 'mp3', source: 'Loyal Books' })),
    ...iaBooks.slice(0, 8).map(b => ({ id: b.id, title: b.title, artist: b.artist || 'Internet Archive', album: b.title, duration: null, artworkURL: b.artworkURL, streamURL: null, format: 'mp3', source: 'Internet Archive' })),
    ...piFeeds.slice(0, 8).map(b => ({ id: b.id, title: b.title, artist: b.artist || 'Podcast Index', album: b.title, duration: null, artworkURL: b.artworkURL, streamURL: null, format: 'mp3', source: 'Podcast Index' }))
  ];

  const albums = mergeDuplicateAlbums(annotateAndSort(q, [...lvAlbums, ...otherAlbums]).filter(x => !/^pg_book_/.test(String(x.id || '')))).slice(0, 24);
  const tracks = mergeDuplicateAlbums(annotateAndSort(q, uniqBy([...lvTracks, ...instantTracks], x => normalizeLoose(x.title + '|' + x.artist)))).slice(0, 14);
...
function buildConfigPage(baseUrl) {
  ...
  h += '<title>Multi Source Audiobook Addon</title>';
  h += '<style>';
  ...
  h += '<div><div class="logo-text">Multi Source</div><div class="logo-sub">Audiobook Addon for Eclipse</div></div></div>';
  ...
  h += '<div class="lbl">Optional Podcast Index API Key</div><input type="text" id="piKey" placeholder="Leave blank unless you want Podcast Index too"><div class="lbl">Optional Podcast Index API Secret</div><input type="password" id="piSecret" placeholder="Leave blank unless you want Podcast Index too"><div class="hint">If entered, your generated URL stores your own Podcast Index credentials with your token, so only your account is used for Podcast Index requests.</div><button class="bo" id="genBtn" onclick="generate()">Generate My Addon URL</button>';
...
app.post('/generate', async function(req, res) {
  ...
  const piSecret = cleanText(req.body && req.body.piSecret);
  if ((piKey && !piSecret) || (!piKey && piSecret)) return res.status(400).json({ error: 'Enter both Podcast Index key and secret, or leave both blank.' });
  if (piKey && piSecret) {
    const valid = await piValidate(piKey, piSecret);
    if (!valid) return res.status(401).json({ error: 'Invalid Podcast Index credentials. Check your key and secret.' });
  }
  const token = generateToken();
  const entry = { createdAt: Date.now(), lastUsed: Date.now(), reqCount: 0, rateWin: [], piKey: piKey || null, piSecret: piSecret || null };
  TOKEN_CACHE.set(token, entry);
...
app.get('/u/:token/search', tokenMiddleware, async function(req, res) {
  ...
  try {
    ...
    const data = await multiSourceSearch(req.tokenEntry, q);
...
app.get('/u/:token/stream/:id', tokenMiddleware, async function(req, res) {
  ...
  if (id.startsWith('ia_book_')) {
    const album = await iaAlbum(id);
    const first = album && Array.isArray(album.tracks) ? album.tracks[0] : null;
    if (first && first.streamURL) return res.json({ url: first.streamURL, format: first.format || 'mp3' });
    return res.status(404).json({ error: 'Internet Archive stream not found.' });
  }

  if (id.startsWith('lb_book_')) {
    const album = await loyalAlbum(id);
    const first = album && Array.isArray(album.tracks) ? album.tracks[0] : null;
    if (first && first.streamURL) return res.json({ url: first.streamURL, format: first.format || 'mp3' });
    return res.status(404).json({ error: 'Loyal Books stream not found.' });
  }

  if (id.startsWith('pi_feed_')) {
    const album = await podcastIndexAlbum(req.tokenEntry, id);
    const first = album && Array.isArray(album.tracks) ? album.tracks[0] : null;
    if (first && first.streamURL) return res.json({ url: first.streamURL, format: first.format || 'mp3' });
    return res.status(404).json({ error: 'Podcast Index stream not found.' });
  }

  if (id.startsWith('ia_tr_') || id.startsWith('lb_tr_') || id.startsWith('pi_ep_')) {
    ...
    return res.status(404).json({ error: 'Cached stream not found.' });
  }
  const cached = CHAPTER_CACHE.get(id);
...
app.get('/u/:token/album/:id', tokenMiddleware, async function(req, res) {
  ...
  if (rawId.startsWith('pi_feed_')) {
    const data = await podcastIndexAlbum(req.tokenEntry, rawId);
    if (!data) return res.status(404).json({ error: 'Podcast Index feed not found.' });
    return res.json(data);
  }
...
app.get('/u/:token/import', tokenMiddleware, async function(req, res) {
  ...
  if (!bookId) {
    const foundBooks = await searchBooks(inputQ);
    const best = foundBooks[0];
    if (!best) return res.status(404).json({ error: 'Book not found. Try a different title.' });
    bookId = String(best.id);
  }
