...
async function multiSourceSearch(entry, q) {
  ...
  const instantTracks = [
    ...loyalBooks.slice(0, 8).map(b => ({ id: b.id, title: b.title, artist: b.artist, album: b.title, duration: null, artworkURL: b.artworkURL, streamURL: null, format: 'mp3', source: 'Loyal Books' })),
    ...iaBooks.slice(0, 8).map(b => ({ id: b.id, title: b.title, artist: b.artist, album: b.title, duration: null, artworkURL: b.artworkURL, streamURL: null, format: 'mp3', source: 'Internet Archive' })),
    ...piFeeds.slice(0, 8).map(b => ({ id: b.id, title: b.title, artist: b.artist, album: b.title, duration: null, artworkURL: b.artworkURL, streamURL: null, format: 'mp3', source: 'Podcast Index' }))
  ];

  const albums = mergeDuplicateAlbums(annotateAndSort(q, [...lvAlbums, ...otherAlbums])).slice(0, 24);
...
app.get('/health', function(req, res) {
  res.json({ status: 'ok', version: '1.3.1', redisConnected: !!(redis && redis.status === 'ready'), podcastIndexConfigured: TOKEN_CACHE.size > 0 ? Array.from(TOKEN_CACHE.values()).some(v => hasPodcastIndex(v)) : false, activeTokens: TOKEN_CACHE.size, cachedSearches: SEARCH_CACHE.size, cachedChapters: CHAPTER_CACHE.size, cachedBooks: BOOK_CACHE.size, timestamp: new Date().toISOString() });
});

app.listen(PORT, () => console.log('Multi Source Audiobook Addon v1.3.1 on port ' + PORT));
