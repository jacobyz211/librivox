...
function mapBookToTrack(book) {
  ...
  const obj = {
    ...
    duration:   book.totaltimesecs ? parseInt(book.totaltimesecs) : null,
    artworkURL: art,
    streamURL:  null,
    format:     'mp3'
  };
...
app.get('/u/:token/search', tokenMiddleware, async function(req, res) {
  ...
  try {
    const [byTitle, byAuthor] = await Promise.all([
      lvGet('/audiobooks', { title: q, limit: 15, extended: 1, coverart: 1 }),
      lvGet('/audiobooks', { author: q, limit: 10, extended: 1, coverart: 1 })
    ]);
    const seen = new Set();
    const allBooks = [...lvBooks(byTitle), ...lvBooks(byAuthor)].filter(b => {
      if (seen.has(String(b.id))) return false;
      seen.add(String(b.id)); return true;
    });

    const albums = allBooks.map(mapBookToAlbum);
    const tracks = allBooks.slice(0, 10).map(mapBookToTrack);
    ...
    allBooks.forEach(book => {
      if (!Array.isArray(book.genres)) return;
      book.genres.forEach(g => {
        if (!g.id || genreMap.has(String(g.id))) return;
        genreMap.set(String(g.id), {
          id:          'lv_genre_' + String(g.id),
          title:       cleanText(g.name) + ' Audiobooks',
          creator:     'LibriVox',
          artworkURL:  null,
          trackCount:  null,
          description: 'Free public domain ' + cleanText(g.name).toLowerCase() + ' audiobooks'
        });
      });
    });
    const playlists = Array.from(genreMap.values()).slice(0, 6);
...
app.get('/u/:token/album/:id', tokenMiddleware, async function(req, res) {
  ...
  try {
    ...
    const genres   = Array.isArray(book.genres) ? book.genres.map(g => g.name) : [];
