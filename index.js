...
async function podcastIndexSearch(entry, q) {
  ...
  try {
    const data = await piGet(entry, '/search/byterm', { q, max: 12, pretty: false });
    const feeds = Array.isArray((data || {}).feeds) ? data.feeds : [];
    return feeds
      .filter(f => /audio/i.test(String(f.medium || '')))
      .map(f => ({
        id: 'pi_feed_' + String(f.id),
        title: cleanText(f.title || f.itunesAuthor || 'Podcast Feed'),
...
async function podcastIndexAlbum(entry, id) {
  ...
  try {
    ...
    const data = await piGet(entry, '/episodes/byfeedid', { id: feedId, max: 200, pretty: false });
    const items = Array.isArray((data || {}).items) ? data.items : [];
    if (!items.length) return null;
    const feedTitle = cleanText(items[0].feedTitle || 'Podcast Feed');
    const artist = cleanText(items[0].feedAuthor || 'Podcast Index');
    const artwork = safeUrl(items[0].feedImage || items[0].image);
    const tracks = items.map((ep, idx) => ({
      id: 'pi_ep_' + String(ep.id || (feedId + '_' + idx)),
      title: cleanText(ep.title || ('Episode ' + (idx + 1))),
      artist,
      album: feedTitle,
    ...
    return { id, title: feedTitle, artist, artworkURL: artwork, trackCount: tracks.length, tracks };
  } catch (e) {
...
function scoreResult(query, title, artist, source, extra = '') {
  ...
  const e = cleanText(extra).toLowerCase();
  ...
  for (const w of words) {
    ...
    else if (a.includes(w) || e.includes(w)) score += 4;
  }
  if (/\b(soundtrack|score|theme|remix|cover|mix|instrumental|karaoke)\b/i.test(t + ' ' + e)) score -= 35;
  if (source === 'LibriVox') score += 8;
  if (source === 'Loyal Books') score += 6;
  if (source === 'Internet Archive') score += 2;
  return score;
}

function annotateAndSort(query, items) {
  return (items || [])
    .map(x => ({ ...x, _score: scoreResult(query, x.title || x.name || '', x.artist || x.creator || '', x.source || '', x.description || '') }))
...
async function multiSourceSearch(entry, q) {
  ...
  const albums = mergeDuplicateAlbums(annotateAndSort(q, [...lvAlbums, ...otherAlbums])).slice(0, 30);
  const tracks = mergeDuplicateAlbums(annotateAndSort(q, uniqBy([...lvTracks, ...instantTracks], x => (x.title + '|' + x.artist).toLowerCase()))).slice(0, 18);
...
app.get('/health', function(req, res) {
  res.json({ status: 'ok', version: '1.2.0', redisConnected: !!(redis && redis.status === 'ready'), podcastIndexConfigured: TOKEN_CACHE.size > 0 ? Array.from(TOKEN_CACHE.values()).some(v => hasPodcastIndex(v)) : false, activeTokens: TOKEN_CACHE.size, cachedSearches: SEARCH_CACHE.size, cachedChapters: CHAPTER_CACHE.size, cachedBooks: BOOK_CACHE.size, timestamp: new Date().toISOString() });
});

app.listen(PORT, () => console.log('Multi Source Audiobook Addon v1.3.0 on port ' + PORT));
